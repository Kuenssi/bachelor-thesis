...
In diesem Beispiel wurden als Begrenzer für das zu ersetzende Stück des Textes die Zeichen \dq <\dq  und \dq >\dq verwendet.
...
In Zeile 6 wird nun abschließend der fertige Text \dq Hello, World!\dq  auf der Konsole ausgegeben.
...
Dies ist je nach Kontext des Templates nötig, da zum Beispiel die Generierung von HTML-Dateien, welche \dq <\dq  und \dq >\dq als Zeichen zum Abgrenzen von Bereichen verwenden,
...
Ein~\ac{ST} wird durch \dq > >\dq geschlossen.
...
JSON-Schemata geben die Struktur und den Inhalt einer JSON-/YAML-Datei vor.
...
\todo{Also sind diese JSON-Schemata eine Art Standard? Woher kommt das? Wer hats erfundn? Anscheinend gibt es ja schon 7 Versionen davon?}

JSON-Schemata können Objektstrukturen in beliebiger Tiefe schachteln.
...
Hierfür ist es möglich bereits erstellte JSON-Schemata im \textit{SchemaStore} bereitzustellen.
Dies ist eine zentrale Stelle, um JSON-Schemata für IDEs zur Verfügung zu stellen.
